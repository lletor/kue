...
/**
 * Get Jobs that match a type and keyword.  //TODO: rewrite using sinter 
 */

exports.getJobDetailGivenSearchAndType = function (req, res) {
	getSearch().query(req.params.searchValue).end(function (err, ids) {
		if (err) return res.json({ error: err.message });
		async.map(ids, Job.get, function (err, unFilteredListOfJobDetails) {
			if (err) return res.json({ error: err.message });
			var result = [];
			for (var i = 0, len = unFilteredListOfJobDetails.length; i < len; i++) {
				if (unFilteredListOfJobDetails[i].type === req.params.jobType) {
					result.push(unFilteredListOfJobDetails[i]);
				}
			}
			res.json(result);
		})
